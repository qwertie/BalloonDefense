...
- The orbit camera can descend to an absolute floor of 0.5 km above the simulated ground, including while zoomed close to the city.
...
- Simulation speed switches automatically between two editable rates. The 20× default remains active while idle, before detection, and while a detected ballistic is still climbing. The 4× rate begins only after a detected ballistic passes apex and, when an engagement is feasible, after an interceptor has actually launched. Playback returns to the fast rate after the ballistic is removed.
...
- Live telemetry reports speed and altitude in both kilometres and feet for every rendered ballistic and interceptor; the fleet-altitude control also shows both units. A ballistic is explicitly marked detected; destroyed or expended tracks remain orange until their explosion animation finishes.
- An airborne explosion cloud inherits the destroyed projectile's velocity vector. Translation and drag use accelerated simulation time, matching the projectile's on-screen speed, while expansion uses display time so it remains visible. Main explosions last about 5.4 display seconds and interceptor bursts about 4 seconds. During the animation the cloud follows a quadratic-drag approximation, `dv/dt = -v²/L`, with stopping length `L = 0.65 + 0.09 × altitude` kilometres (minimum 0.6 km). A ground impact instead produces a larger stationary two-layer burst and reports zero post-impact velocity so it cannot disappear below the terrain. These are visual heuristics, not fragment-dispersion models.
